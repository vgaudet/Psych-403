...
filename = str(exp_info['subject_nr']) + '_' + exp_info['date'] + '_'  + str(exp_info['session'])
...
# 200 x 200 image size, 2 second stimulus presentation, 1 second cross presentation
stimheight = 200
stimwidth = 200
stimdur= 2
crossdur= 1
...
exprec = [] # empty list that will contain experiment blocks
...
block_msg = "Press any key to continue to the next block.\n...\n Press %s when you see a face." % ans_key[0]
...
for block in range (blocks): #-for loop for nBlocks *
    ...
    #assume images with a face are labelled with face in title
    #correct response is the designated answer key for face image
    for pic in pics:
        if "face" in pic:
            key.append(ans_key)
        else:
    ...
    blockrec = {} # initialize block dict
 ...
 #   '''
    for trial in range(trials): #-for loop for nTrials *
        ...
        # if subject response matches key, correct
        # if subject response does not match response, incorrect
        # if subject response is empty, incorrect
        # if subject response is empty and key is empty (no face in stim), correct
        if sub_resp == key[trial]:
            resp_acc = 1
        elif not sub_resp and (key[trial] == ['None']):
            sub_resp = ['None']
            resp_acc = 1
            resp_time = None
        elif not sub_resp and (key[trial] != ['None']):
        ...
        # record trial into block dict
        blockrec[trial] = {'trial' : trial+1,
                        'key name': sub_resp, 
                        'subject RT' : resp_time, 
                        'subject accuracy' : resp_acc,
                        'correct response' : key[trial]
...
#        '''
    # block timers end
    exprec.append(blockrec) # add block dict to experiment list containing all the blocks
...
fields = ['trial', 'key name', 'subject RT', 'subject accuracy', 'correct response']
 
# save each block into different csv file
for block in exprec:
    data_dir = os.path.join(main_dir, 'data', 'b%i_' % exprec.index(block)+ filename + '.csv' )    
    with open(data_dir, 'w', newline  = '') as csvfile:
        writer = csv.DictWriter( csvfile, fields )
        writer.writeheader()
        for trial in block:
            writer.writerow({item: block[trial].get(item) for item in fields}) # Reference: User: Łukasz Rogalski, Apr 1, 2015 at 21:05 https://stackoverflow.com/questions/29400631/python-writing-nested-dictionary-to-csv
        
# save each block into different json file
for block in exprec:
    data_dir = os.path.join(main_dir, 'data', 'b%i_' % exprec.index(block)+ filename + '.json')  
    jsonblock = []
    for trial in block:
        jsonblock.append(block[trial])
    with open(data_dir, 'w', newline  = '\n') as jsonfile:
        json.dump(jsonblock, jsonfile)


#======================
# EXPERIMENT ANALYSIS
#======================  
#Json read and analysis by block file
for block in range(blocks):
    print('\n')
    data_dir = os.path.join(main_dir, 'data', 'b%i_' % block + filename + '.json')  
    df = pd.read_json(data_dir)
    print("Experiment Block %d Summary:\n" % (block+1), df)
    df = df.loc[(df['subject accuracy'] != -1)] # remove trials with no response
    df['subject RT'].mask(df['subject accuracy'] ==  0, None, inplace=True) # replace RT where response is incorrect to None
    print('Block %d post filter:\n' % (block+1), df)
    print('Means excluding no response and incorrect response RT:\n', df[['subject RT', 'subject accuracy', 'correct response']].mean(axis=0, skipna = True), '\n') # get column means, skip None or Null values
